...
var StateMachine = stampit({
  ...
  methods: {
    ...
    addConditionalEvent: function addConditionalEvent (event) {
      ...
      this.callbacks[callbackPrefix + 'entered' + pseudoState] = this.enteredPseudoStateCallback(pseudoState, event).bind(this);
    },
    enteredPseudoStateCallback: function enteredPseudoStateCallback(pseudoState, event) {
      var pseudoEvent = this.factory.pseudoEvent;
        ...
        return new Promise(function (resolve) {
          ...
          .then(function (index) {
            ...
            if (_.isUndefined(toState)) {
              return target[pseudoEvent(pseudoState, this.factory.noChoiceFound)]()
    ...
    addEventId: function addEventId(options) {
      if (options.from === options.to) {
        options.id = uuid();
      }
    },
    userLeaveCallback: function userLeaveCallback(options) {
      var callbackName = this.factory.callbackPrefix + 'leave' + this.current;

      return _.get(this.callbacks, callbackName, _.identity).bind(this.target, options);
    },
    userEnterStageCallback: function userEnterStageCallback(options, name, stage) {
      var callbackName = this.factory.callbackPrefix + stage + this.events[name][this.current];

      return _.get(this.callbacks, callbackName, _.identity).bind(this.target, options);
    },
    commonEnterStageCallback: function commonEnterStageCallback (options, stage) {
      var callback = this.callbacks['on' + stage];

      if (callback && !this.pseudoStates[options.to]) {
        return callback.bind(this.target, options);
      }
    ...
    eventFactory: function eventFactory (name) {
      return function triggerEvent () {
        ...
        var options = {
          name: name,
          from: current,
          to: this.events[name][current],
          args: args
        };
        var pOptions;
        var isPseudo = this.pseudoEvents[name];

        this.addEventId(options);

        if (this.pseudoStates[options.to]) {
        ...
        return new this.factory.Promise(function (resolve) {
          ...
          .then(this.userLeaveCallback(options))
          .then(_.get(this.callbacks, 'onleave', _.identity).bind(this.target, options))
          .then(this.onleavestate.bind(this, options))
          .then(_.get(this.callbacks, this.factory.callbackPrefix + name, _.identity).bind(this.target, options))
          //in the case of the transition from choice pseudostate we provide
          // the options of the original transition
          .then(this.userEnterStageCallback(isPseudo ? pOptions : options, name, 'enter'))
          .then(this.commonEnterStageCallback(isPseudo ? pOptions : options, 'enter'))
          .then(this.onenterstate.bind(this, options))
          .then(this.userEnterStageCallback(isPseudo ? pOptions : options, name, 'entered'))
          .then(this.commonEnterStageCallback(isPseudo ? pOptions : options, 'entered'))
          .then(this.returnValue.bind(this, options))
          .catch(this.revert(options));
      }.bind(this);
    },
...
var Composition = AssignFirstArgument
